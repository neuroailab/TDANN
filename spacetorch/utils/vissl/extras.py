...
# add SyncNormalizeFunction since it isn't yet included in VISSL release
class SyncNormalizeFunction(Function):
    ...
    def forward(ctx, input, eps):
        with torch.no_grad():
            ...
            # If running on a distributed setting, perform mean reduction of tensors
            # over all processes.
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            # If running on a distributed setting, perform mean reduction of
            # tensors over all processes.
