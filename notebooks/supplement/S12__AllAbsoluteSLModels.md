...
from spacetorch.utils import figure_utils, plot_utils, seed_str
...
```python
sw0_base = "simclr/simclr_spatial_resnet18_swappedon_SineGrating2019_isoswap_3"
abs_base = "simclr/simclr_spatial_resnet18_swappedon_SineGrating2019_old_scl"
SWS = {
    "SW_0": ("_lw0", sw0_base),
    "SW_0pt1": ("_lw01", abs_base),
    "SW_0pt25": ("", abs_base),
    "SW_0pt5": ("_lwx2", abs_base),
    "SW_1pt25": ("_lwx5", abs_base),
    "SW_2pt5": ("_lwx10", abs_base),
    "SW_25": ("_lwx100", abs_base),
}
```
...
tissues = {
    "V1": {
        ...
        for sw, (lwmod, base) in SWS.items()
    },
    "VTC": {
        sw: {
...
figure_utils.save(fig, "S09/orientation_maps")
```
```python
...
for col_idx, (name, seeds) in enumerate(tissues["VTC"].items()):
    for row_idx, (seed, tissue) in enumerate(tqdm(seeds.items())):
        ...
        if col_idx == 0:
            ax.text(-0.25, 0.2, f"Seed {seed}", rotation=90, transform=ax.transAxes)
figure_utils.save(fig, "S09/sel_maps")
```
